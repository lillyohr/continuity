...
export function registerStartCommand(program) {
    program
        ...
        try {
            getDb(root).prepare(`INSERT OR IGNORE INTO jobs (job_id, slug, title, created_at, status)
           VALUES (?, ?, ?, ?, 'active')`).run(identity.jobId, identity.slug, identity.jobName, identity.createdAt);
        }
        catch (err) {
            console.error(`Warning: could not update state database: ${err.message}`);
