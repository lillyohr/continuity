...
function currentSessionId(root, slug) {
    try {
        const row = getDb(root).prepare(`SELECT id FROM sessions
       WHERE job_id = (SELECT job_id FROM jobs WHERE slug = ?)
         AND ended_at IS NULL
       ORDER BY id DESC LIMIT 1`).get(slug);
        return row?.id ?? null;
    }
    catch {
        return null;
...
function flagSessionHasEdits(root, slug) {
    try {
...
function resetSessionHasEdits(root, slug) {
    try {
        const id = currentSessionId(root, slug);
        if (id == null)
            return;
...
function getSessionHasEdits(root, slug) {
    try {
        const id = currentSessionId(root, slug);
        if (id == null)
            return false;
        const row = getDb(root).prepare(`SELECT has_edits FROM sessions WHERE id = ?`).get(id);
...
function recordStopSyncRequested(root, slug) {
    try {
        const id = currentSessionId(root, slug);
        if (id == null)
            return;
        getDb(root).prepare(`UPDATE sessions SET stop_sync_requested_at = ? WHERE id = ?`).run(new Date().toISOString(), id);
    }
    catch { /* non-fatal */ }
}
function renderSyncInstruction(slug) {
...
export function registerHookCommand(program) {
    program
        ...
        if (event === "stop") {
            ...
            if (!job)
                process.exit(0);
            if (!getSessionHasEdits(root, job.slug))
                process.exit(0);
        ...
        if (event === "session-start") {
            if (job) {
                ...
                process.stdout.write(`Continuity: attached to ${job.slug}. Read continuity/jobs/${job.slug}/HANDOFF.md before continuing.\n`);
            }
            return;
        }
        ...
        if (event === "pre-compact" && job) {
            process.stdout.write(`[CONTINUITY] Compaction detected for job "${job.slug}". Update HANDOFF.md if needed before context is lost.\n`);
        }
    });
}
