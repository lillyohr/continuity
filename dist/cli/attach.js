...
function readJobId(root, slug) {
    ...
    try {
        const row = getDb(root).prepare(`SELECT job_id FROM jobs WHERE slug = ?`).get(slug);
...
export function registerAttachCommand(program) {
    program
        ...
        try {
            ...
            db.prepare(`INSERT OR IGNORE INTO jobs (job_id, slug, title, created_at, status)
           VALUES (?, ?, ?, ?, 'active')`).run(jobId, slug, slug, new Date().toISOString());
            db.prepare(`INSERT INTO sessions (job_id, started_at) VALUES (?, ?)`).run(jobId, new Date().toISOString());
        }
        catch {
